...
We're given a Python script (`chall.py`) that implements a lattice-based cryptographic scheme and an output file containing the public parameters and encrypted flag.
...
We construct a lattice that encodes both constraints:
1. `s = z·Bs` (s lies in the Bs lattice)
2. `b = s·A + e` (LWE relation with small error)

Substituting, we get: `z·(Bs·A) + e ≡ b (mod q)`

We build the embedding lattice:
...
1. **Lattice-based crypto vulnerabilities**: When secrets are constrained to be small, lattice reduction (LLL/BKZ) can often recover them
2. **Kannan embedding**: A powerful technique to convert search problems into finding short vectors
3. **Scaling matters**: Proper scaling of different components in the lattice is crucial for BKZ to find the target vector
4. **Structure exploitation**: The special form of `Bs` (block structure with identity matrices) makes the attack feasible
