...
The service prints three 32-bit "glimpses" (the upper 32 bits) of a 64-bit RNG state, with a large "jump across time" between glimpses, then asks for the next 5 full 64-bit states.

## Given RNG

From the provided script (`fortune_revenge(1).py`), the Fortune Teller uses a 64-bit LCG:
