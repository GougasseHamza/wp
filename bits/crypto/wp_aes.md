...
So the AES key is 16 bytes, and we know 13 bytes → **only 3 bytes are unknown**:
- Search space = `2^(8*3) = 2^24 = 16,777,216` candidates.
That’s totally brute-forceable with a fast check.

---

## Attack plan

1. Parse `output(1).txt`:
   - read `key_hint`
   - read one (or a few) plaintext/ciphertext sample pairs
   - read `encrypted_flag`
2. Brute-force the last 3 key bytes:
